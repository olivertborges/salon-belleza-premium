...
export default function PeluqueriaPage() {
  ...
  const handleSubmitReview = async () => {
    ...
    setSubmitting(true)
    try {
      // 1. Obtener el client_id correspondiente al auth user id
      ...
      const clientIdToInsert = clientData?.id || user.id

      // 2. Realizar la inserción
      const { data, error } = await supabase
        .from('reviews')
        .insert({
      ...
      setErrorMessage(error.message || 'Error al enviar la calificación')
